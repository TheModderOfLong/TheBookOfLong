    ...
    internal static class SpeHeroSkeletonOverrideRenderer
    {
        ...
        private static bool TryApplySpineSkeleton(HeroData heroData, SpeHeroSkeletonOverrideEntry entry, Transform targetSkeletonParent)
        {
            ...
            if (speSkeletonTransform == null)
            {
                ...
                ApplySpeSkeletonInitialPosition(targetSkeletonParent, rt, zOffset);
                ApplySpeSkeletonContainerScale(targetSkeletonParent, rt);
        ...
        {
            Transform reference = FindLayoutReferenceTransform(targetSkeletonParent, null);
            return reference == null ? 1f : reference.localScale.x;
        }

        private static Transform FindLayoutReferenceTransform(Transform targetSkeletonParent, Transform excluded)
        {
            if (targetSkeletonParent == null || targetSkeletonParent.childCount <= 0) return null;

            for (int i = 0; i < targetSkeletonParent.childCount; i++)
            {
                Transform child = targetSkeletonParent.GetChild(i);
                if (!CanUseLayoutReference(child, excluded)) continue;

                SkeletonGraphic skeleton = child.GetComponent<SkeletonGraphic>();
                if (skeleton != null && child.gameObject.activeSelf) return child;
            }

            for (int i = 0; i < targetSkeletonParent.childCount; i++)
            {
                Transform child = targetSkeletonParent.GetChild(i);
                if (CanUseLayoutReference(child, excluded)) return child;
            }

            return null;
        }

        private static bool CanUseLayoutReference(Transform child, Transform excluded)
        {
            if (child == null || child == excluded) return false;
            if (child.name == OverrideRootName) return false;
            return true;
        }

        private static void ApplySpeSkeletonInitialPosition(Transform targetSkeletonParent, Transform speSkeletonTransform, float zOffset)
        {
            if (speSkeletonTransform == null) return;

            speSkeletonTransform.localPosition = new Vector3(0f, 0f, zOffset);
            Transform reference = FindLayoutReferenceTransform(targetSkeletonParent, speSkeletonTransform);
            if (reference == null) return;

            speSkeletonTransform.localPosition = new Vector3(reference.localPosition.x, reference.localPosition.y, zOffset);
        }
        ...
        private static void ApplySpeSkeletonVanillaScale(Transform targetSkeletonParent, Transform speSkeletonTransform)
        {
            ...
            Transform reference = FindLayoutReferenceTransform(targetSkeletonParent, speSkeletonTransform);
            float scale = reference == null ? 1f : reference.localScale.x;
            if (Mathf.Approximately(scale, 0f)) return;

            speSkeletonTransform.localScale = Vector3.one * scale;
        }

        private static void ApplySpeSkeletonContainerScale(Transform targetSkeletonParent, Transform speSkeletonTransform)
        {
