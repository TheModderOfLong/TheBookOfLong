...
namespace TheResourceOfLong
{
    internal static class SpeHeroSkeletonOverrideRenderer
    {
        ...
        public static bool TryApply(HeroData heroData, Transform targetSkeletonParent)
        {
            ...
            // 图片/预制体在需要翻转的容器中做容器级翻转（Spine 由原生初始化自带翻转）
            // 对需要翻转的容器做容器级翻转（翻转 OverrideRoot，SpeSkeleton 不在其内故不受影响）
            if (NeedsContainerFlip(targetSkeletonParent))
            {
        ...
        private static bool IsDetailFaceContainer(Transform transform)
        {
            return string.Equals(GetTransformPath(transform), DetailFaceContainerPath, StringComparison.Ordinal);
            string path = GetTransformPath(transform);
            return path.Contains(DetailFaceContainerPath);
        }

        private static bool NeedsContainerFlip(Transform targetSkeletonParent)
        {
            return IsDetailFaceContainer(targetSkeletonParent) || IsHudFaceContainer(targetSkeletonParent);
        }
        ...
        /// HUD 路径额外调用 ApplyHudSpeSkeletonTemplateOffset 做专属偏移。
        /// </summary>
        private static bool FinalizeSpeSkeleton(SpeHeroSkeletonOverrideEntry entry, SkeletonGraphic skeletonGraphic, SkeletonDataAsset skeletonDataAsset, Transform targetSkeletonParent)
        {
            ...
            // 从同级已有骨架（HeroSkeleton0/1）复制 Skeleton.ScaleX，保证朝向一致
            if (targetSkeletonParent != null && skeletonGraphic.Skeleton != null)
            {
                for (int ci = 0; ci < targetSkeletonParent.childCount; ci++)
                {
                    SkeletonGraphic refSg = targetSkeletonParent.GetChild(ci).GetComponent<SkeletonGraphic>();
                    if (refSg != null && refSg != skeletonGraphic && refSg.Skeleton != null)
                    {
                        skeletonGraphic.Skeleton.ScaleX = refSg.Skeleton.ScaleX;
                        break;
                    }
                }
            }
        ...
        private static bool TryApplySpineSkeleton(HeroData heroData, SpeHeroSkeletonOverrideEntry entry, Transform targetSkeletonParent)
        {
            ...
            if (speSkeletonTransform == null)
            {
                ...
                float zOffset = heroData.isFemale ? -0.062f : -0.072f;
                // 从第一个真实子节点复制 x/y 坐标，仅覆盖 z 偏移
                rt.localPosition = new Vector3(0f, 0f, zOffset);
                for (int ci = 0; ci < targetSkeletonParent.childCount; ci++)
                {
                    Transform child = targetSkeletonParent.GetChild(ci);
                    if (child == null || child.name == "TheResourceOfLongMappingOverride") continue;
                    rt.localPosition = new Vector3(child.localPosition.x, child.localPosition.y, rt.localPosition.z);
                    break;
                }
                float scale = GetFirstChildScaleX(targetSkeletonParent);
        ...
        private static bool TryApplyHudSpineSkeletonCompatibilityLayer(HeroData heroData, SpeHeroSkeletonOverrideEntry entry, Transform targetSkeletonParent, SkeletonDataAsset skeletonDataAsset)
        {
            ...
            if (!IsHudFaceContainer(targetSkeletonParent))
            {
            ...
            SkeletonGraphic skeletonGraphic = SkeletonGraphic.NewSkeletonGraphicGameObject(skeletonDataAsset, root.transform, mat);
            if (skeletonGraphic == null)
            {
        ...
        private static void ApplyTransformFlipX(SpeHeroSkeletonOverrideEntry entry, Transform transform, float baseScaleX)
        {
            ...
            LoggerManager.Debug($"[FlipX] entry.FlipX={entry.FlipX} path={GetTransformPath(transform)} baseScaleX={baseScaleX} before=({scale.x}), after=({scale.x})");
        ...
        private static void ApplySkeletonFlipX(SpeHeroSkeletonOverrideEntry entry, SkeletonGraphic skeletonGraphic)
        {
            ...
            StoreOriginalSkeletonGraphicScaleX(skeletonGraphic);
            float scaleX = skeletonGraphic.Skeleton.ScaleX;
            if (Mathf.Abs(scaleX) <= 0f) scaleX = 1f;
        ...
        private static bool IsHudFaceContainer(Transform transform)
        {
            return string.Equals(GetTransformPath(transform), HudFaceContainerPath, StringComparison.Ordinal);
        }
