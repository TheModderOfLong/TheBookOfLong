...
namespace TheExtensionOfLong
{
    /// <summary>
    /// 判断两个角色是否为恋人关系。
    /// 格式: [$IsPreLover:源角色ID/名称:目标角色ID/名称$]
    /// 角色参数为空时分别默认使用 sourceInteractHero 与 targetInteractHero。
    /// </summary>
    [ConditionQuery("IsPreLover")]
    public static class QueryIsPreLover
    {
        public static string TryQuery(PlotController plotController, string[] parts)
        {
            ...
            return (source != null && target != null && (source.HavePrelover(target.heroID) || target.HavePrelover(source.heroID))) ? "1" : "0";
        }
    }
}
