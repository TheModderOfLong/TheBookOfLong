...
{
    /// <summary>
    /// 根据任务名称获取主线任务并交给GameController处理
    /// 在 MainMissionDataBase 中查找 MissionData.name 匹配的任务，Clone 后调用 GetFullMission
    /// 与原版 GetMainMission(int) 按索引取值不同，此指令按 name 字段搜索
    /// 格式: GetMainMissionByName*任务名称
    /// 示例: GetMainMissionByName*主线任务A
    /// </summary>
    [SpePlotFuc("GetMainMissionByName")]
    public static class SpePlotFucGetMainMissionByName
    {
        public static void TryCall(PlotController plotController, string fucName, string[] fucParams)
        {
            ...
            if (found == null)
            {
                LoggerManager.Warning($"{fucName}: 未找到名称为\"{missionName}\"的主线任务");
                return;
            }
            ...
            LoggerManager.Debug($"{fucName}: 已获取主线任务 name={missionName}, speMissionID={cloned.speMissionID}");
        }
    }
}
