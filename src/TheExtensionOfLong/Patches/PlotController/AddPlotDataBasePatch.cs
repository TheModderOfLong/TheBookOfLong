...
{
    [HarmonyPatch(typeof(PlotController), "AddPlotDataBase", new[] { typeof(string) })]
    public class AddPlotDataBasePatch
    {
        [HarmonyPrefix]
        public static bool AddPlotDataBasePrefix(PlotController __instance, string plotID)
        {
            ...
            else
            {
                if (SymbolicIdService.TryResolveId(plotID, out int assignedId))
                {
                    ...
                    __instance.AddPlotDataBase(assignedId);
                    return false;
                }
                else
                {
